...
import { useDeleteFoodReviewMutation, useGetFoodReviews } from "@/queries";
...
const FoodReviews = () => {
  ...
  const { data: reviews } = useGetFoodReviews(param.slug as string);
  const { mutateAsync: deleteMutate } = useDeleteFoodReviewMutation();
    ...
    <>
      ...
      <div className="flex items-center h-[50px] mt-4 p-2 border border-tag rounded-lg">
        ...
        <Button
          ...
          onClick={() => router.push(`${PATHNAME.FOOD}/rating/${param.slug}`)}
        >
          <Plus />
        </Button>
      </div>
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-20 p-0 bg-background">
                      <Button
                        ...
                        onClick={() =>
                          router.push(
                            `${PATHNAME.FOOD}/rating/${param.slug}/?reviewId=${review.id}`
                          )
                        }
                      >
