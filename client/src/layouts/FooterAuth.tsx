...
const FooterAuth = () => (
  <div className="fixed bottom-4 w-full px-6 max-w-[390px]">
    ...
    <div className="flex justify-between">
      <Button className="w-36 h-14 rounded-[28px] p-4 bg-white shadow-socialBtn">
        <Image
          alt={IMAGES_CONST.auth.facebookIcon.toString()}
          src={IMAGES_CONST.auth.facebookIcon}
          className="w-9 h-9"
        />
        <span className="w-24 text-black">FACEBOOK</span>
      </Button>
      <Button className="w-36 h-14 rounded-[28px] p-4 bg-white shadow-socialBtn">
        <Image
          ...
          className="w-8 h-8"
        />
        <span className="w-24 text-black">GOOGLE</span>
      </Button>
    </div>
  </div>
);
